...
```{r}
# Asumimos que 'n_muestras' es el número de filas en tu dataframe_model
n_muestras <- nrow(dataframe_model) 

# 1. Crear un único dataframe "largo" para ggplot
...
# 2. Graficar ambas densidades
ggplot(plot_data, aes(x = valor, fill = tipo_prediccion)) +
  
  # geom_density() crea el gráfico de densidad
  # alpha = 0.7 lo hace semi-transparente
  ...
  labs(
    ...
    fill = "Tipo de Simulación"  # Etiqueta para la leyenda
  ) +
  theme_minimal()
```
